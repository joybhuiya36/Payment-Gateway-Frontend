...
const add = (pre: any, newData: any) => {
  const existance = pre.findIndex((x: any) => x._id === newData._id);
...
const remove = (pre: any, id: any) => {
  const idx = pre.findIndex((x: any) => x._id === id);
...
const removeManyFunc = (pre: any, id: any, quantity: number) => {
  const idx = pre.findIndex((x: any) => x._id === id);
